...
struct rtl_tx_descriptor {
    ...
    uint16 flags;
    uint32 vlan;
    uint32 tx_buffer_low;
    uint32 tx_buffer_high;
};

struct rtl_rx_descriptor {
    union {
        uint16 buffer_size; /* rx command */
        uint16 frame_len;   /* rx status - top 2 bits are checksum offload flags */
    };
    uint16 flags;
    uint32 vlan;
    uint32 rx_buffer_low;
    uint32 rx_buffer_high;
};
