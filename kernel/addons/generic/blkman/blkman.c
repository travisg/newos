	...
	size_t dma_boundary, bool dma_boundary_solid )
{
	...
	for( cur_idx = 0; cur_idx < map->num; ++cur_idx ) {
		addr dma_end, dma_len;
				
		dma_end = ((addr)map->vec[cur_idx].start + map->vec[cur_idx].len 
	...
	size_t vec_offset, uint alignment )
{
	...
	{
		if( (((addr)vecs->start + vec_offset) & alignment) != 0 )
			return false;
			
		if( (((addr)vecs->start + vecs->len) & alignment) != 0 )
			return false;
...
int vm_lock_range( addr vaddr, size_t len, int flags );
int vm_unlock_range( addr vaddr, size_t len );
	...
	size_t vec_offset, size_t len, int flags )
{
	...
	for( ; len > 0; ++vecs, --num_vecs ) {
		...
		if( vm_lock_range( (addr)vecs->start + vec_offset, 
	...
	size_t vec_offset, size_t len )
{
	...
	for( ; len > 0; ++vecs, --num_vecs ) {
		...
		vm_unlock_range( (addr)vecs->start + vec_offset, lock_len );
	...
	size_t vec_count, off_t pos, ssize_t len, bool write )
{
	...
	if( (res = vm_lock_range( (addr)vec, vec_count * sizeof( vec[0] ), 0 )) != NO_ERROR )
		return res;
	...
	vm_unlock_range( (addr)vec, vec_count * sizeof( vec[0] ));
...
static int blkman_init_buffer( void )
{
	...
	res = vm_get_page_mapping( vm_get_kernel_aspace_id(), (addr)blkman_buffer, 
		&blkman_buffer_phys );
...
static int vm_get_page_range_mapping( vm_address_space *aspace, addr vaddr, size_t len,
	phys_vec *map, size_t max_entries, size_t *num_entries, size_t *mapped_len )
{
	size_t cur_idx = 0;
	size_t left_len = len;
	...
	phys_vec *map, size_t max_entries, size_t *num_entries, size_t *mapped_len )
{
		...
		++vec, --vec_count ) 
	{
		...
		range_start = (addr)vec->start + vec_offset;
