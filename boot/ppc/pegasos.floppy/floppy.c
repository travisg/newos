...
static ssize_t floppy_read(floppy *flp, void *_buf, off_t pos, ssize_t len)
{
	...
	if((pos % SECTOR_SIZE) != 0) {
		...
		err = read_sectors(flp, buf, pos / SECTOR_SIZE, 1);
		if(err <= 0)
			goto out;
		...
		memcpy(_buf, buf + (pos % SECTOR_SIZE), toread);
		len -= toread;
		bytes_read += toread;
		pos += toread;
	}
	...
	while(len >= SECTOR_SIZE) {
		...
		err = read_sectors(flp, (char *)_buf + bytes_read, pos / SECTOR_SIZE, len / SECTOR_SIZE);
		if(err <= 0)
			goto out;
	...
	if(len > 0 && (len % SECTOR_SIZE) != 0) {
		...
		memcpy((char *)_buf + bytes_read, buf, len);
...
static ssize_t floppy_write(floppy *flp, const void *_buf, off_t pos, ssize_t len)
{
	...
	if((pos % SECTOR_SIZE) != 0) {
		...
		memcpy(buf + (pos % SECTOR_SIZE), _buf, towrite);

		/* write this sector back out */
		err = write_sectors(flp, buf, pos / SECTOR_SIZE, 1);
		if(err <= 0)
			goto out;
	...
	while(len >= SECTOR_SIZE) {
		...
		err = write_sectors(flp, (char *)_buf + bytes_written, pos / SECTOR_SIZE, len / SECTOR_SIZE);
		if(err <= 0)
			goto out;
	...
	if(len > 0 && (len % SECTOR_SIZE) != 0) {
		...
		/* copy the modified data over */
		memcpy(buf, (char *)_buf + bytes_written, len);
