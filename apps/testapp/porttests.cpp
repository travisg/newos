...
void port_test(void)
{
    ...
    test_p1 = _kern_port_create(1,    "test port #1");
    test_p2 = _kern_port_create(10,   "test port #2");
    test_p3 = _kern_port_create(1024, "test port #3");
    test_p4 = _kern_port_create(1024, "test port #4");

    printf("porttest: port_find()\n");
    printf("'test port #1' has id %d (should be %d)\n", _kern_port_find("test port #1"), test_p1);

    printf("porttest: port_write() on 1, 2 and 3\n");
    _kern_port_write(test_p1, 1, &testdata, sizeof(testdata));
    _kern_port_write(test_p2, 666, &testdata, sizeof(testdata));
    _kern_port_write(test_p3, 999, &testdata, sizeof(testdata));
    printf("porttest: port_count(test_p1) = %d\n", _kern_port_count(test_p1));

    printf("porttest: port_write() on 1 with timeout of 1 sec (blocks 1 sec)\n");
    _kern_port_write_etc(test_p1, 1, &testdata, sizeof(testdata), PORT_FLAG_TIMEOUT, 1000000);
    printf("porttest: port_write() on 2 with timeout of 1 sec (wont block)\n");
    res = _kern_port_write_etc(test_p2, 777, &testdata, sizeof(testdata), PORT_FLAG_TIMEOUT, 1000000);
    printf("porttest: res=%d, %s\n", res, res == 0 ? "ok" : "BAD");

    printf("porttest: port_read() on empty port 4 with timeout of 1 sec (blocks 1 sec)\n");
    res = _kern_port_read_etc(test_p4, &dummy, &dummy2, sizeof(dummy2), PORT_FLAG_TIMEOUT, 1000000);
    printf("porttest: res=%d, %s\n", res, res == ERR_PORT_TIMED_OUT ? "ok" : "BAD");

    printf("porttest: spawning thread for port 1\n");
    t = _kern_thread_create_thread("port_test", port_test_thread_func, NULL);
    // resume thread
    _kern_thread_resume_thread(t);
    ...
    _kern_port_write(test_p1, 2, &testdata, sizeof(testdata));
    printf("porttest: write #3\n");
    _kern_port_write(test_p1, 3, &testdata, sizeof(testdata));

    printf("porttest: waiting on spawned thread\n");
    _kern_thread_wait_on_thread(t, NULL);

    printf("porttest: close p1\n");
    _kern_port_close(test_p2);
    printf("porttest: attempt write p1 after close\n");
    res = _kern_port_write(test_p2, 4, &testdata, sizeof(testdata));
    printf("porttest: port_write ret %d\n", res);

    printf("porttest: testing delete p2\n");
    _kern_port_delete(test_p2);
